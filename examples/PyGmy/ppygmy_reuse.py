...
class world:
    ...
    def render_row(self, sy):
        ...
        """
        Render the given row, using 'sy' as the row position, and returning a
        tuple containing 'sy' and a list of result numbers.
        """
    ...
    def render(self, filename, limit):
        ...
        exchange = PyGmyExchange(self.width, self.height, image, limit=limit, reuse=1)
...
class PyGmyExchange(pprocess.Exchange):
    ...
    def __init__(self, width, height, image, *args, **kw):

        """
        Initialise the exchange, adding extra PyGmy-specific data such as the
        'width' and 'height' of the eventual 'image'.
        """

        pprocess.Exchange.__init__(self, *args, **kw)
